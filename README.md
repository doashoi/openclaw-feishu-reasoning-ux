...
- 使用 **OpenClaw 内置的 Feishu 通道**
- 想改善飞书里的消息卡片体验，或者排查 raw reasoning / streaming 问题

需要特别注意：

- 不要把它和 **飞书官方插件**、其他 Feishu 集成、浏览器插件方案混为一谈
- 不同 OpenClaw 版本、不同 provider 路线、不同实际运行代码路径，结果都可能不一样
- 如果当前模型路径不支持 readable live reasoning，这个 skill 也不会把它强行“变出来”
- 真正关键的不是模型名字本身，而是这条路径有没有实际产出可用的 live reasoning 信号

WSL 是我们验证案例的一部分，但不是唯一前提。真正更重要的是：

- 当前 OpenClaw 版本 / build
- 当前 Feishu 通道实现
- 当前 provider/model 路线
- 当前路径是否实际产出了可读的 live reasoning 信号
- 当前实际加载的运行代码路径
...
这是一个专门用来改善 Openclaw 在飞书通道中回复体验的 skill。

它的目标是：

- 帮你把飞书里的回复做得更可见、更有层次
- 在支持的情况下显示真实 raw reasoning
- 在不支持的情况下，继续优化卡片和流式回复体验
